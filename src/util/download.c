...
#define TRY_DOWNLOAD(_name_)  extern unsigned char media_##_name_##_raw[]; \
                                      extern size_t media_##_name_##_raw_len; \
                                      download(#_name_, media_##_name_##_raw_len, media_##_name_##_raw);


void download(const char* fname, size_t len, char* data) {
    printf("Downloading %s", fname);
    char full_name[32];
    memcpy(full_name, fname, sizeof(full_name));
    // Add extension
    strlcat(full_name, ".raw", sizeof(full_name));

    FIL fi;
    FRESULT res;
    res = f_open(&fi, full_name, FA_WRITE | FA_CREATE_ALWAYS);
    if (res != FR_OK) {
    ...
    res = f_write(&fi, data, len, &nwritten);
    if (res != FR_OK || nwritten != len) {
        printf("Could not write to file %s\n", fname);
    } else {
        printf("Wrote %d bytes to %s/%s\n", nwritten, DIRECTORY, fname);
    }
    f_close(&fi);
}
...
//     if (res != FR_OK) {
//         printf("Warning: could not make directory\n");
//     }
...
//     #ifdef DOWNLOAD_snare
//     TRY_DOWNLOAD(snare);
//     #endif

//     #ifdef DOWNLOAD_hello_world
//     TRY_DOWNLOAD(hello_world);
//     #endif

//     printf("Loading all audio files\n");
//     loadAllAudioFiles();
// }
